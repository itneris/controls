...
import {
    ...
    TextControl,
    TimeControl
} from '@itneris/controls-react'
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import { rolesDictionary, testUser, UserDTO } from './demoDataForm';
...
function DemoForm() {
    ...
    const { data: roles, ...rolesQuery } = useQuery({
        queryKey: ['roles'],
        queryFn: async () => {
            await new Promise(res => setTimeout(res, 400));
    ...
    const { mutate: saveUser, ...saveMutation } = useMutation({
    ...
    const changeUser = useCallback((user: UserDTO) => {
        queryClient.setQueryData(['user_info'], user);
    }, [queryClient]);

    const handleSave = useCallback((user: UserDTO) => {
        saveUser(user);
    }, [saveUser]);
    ...
    return (
        <ItnForm
            ...
            errorLoading={userQuery.error?.message}
            onEntityChange={changeUser}
            isSaving={saveMutation.isPending}
            onSave={handleSave}
        >
            <TextControl label="Name" name="name" required />
            <NumberControl label="Visit count" name="visitCount" required />
            <SelectControl items={roles} label="Role" name="primaryRole" loading={rolesQuery.isFetching} required />
            <DateControl label="Visit date" name="visitDate" required />
            <DateTimeControl label="Visit date and time" name="visitDateTime" required />
            <TimeControl label="Visit time" name="visitTime" required />
            <SelectControl multiple items={roles} label="Roles" name="roles" loading={rolesQuery.isFetching} required />
            <BoolControl label="Blocked" name="blocked" />
            <PasswordControl label="Password" name="password" required />
            <TextControl label="Note" name="note" required multiline />
        </ItnForm>
    )
}
